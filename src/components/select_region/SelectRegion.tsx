...
const SelectRezion = ({ onGetRegionNumber }: Props) => {
	const [selectedAddr1Num, setSelectedAddr1Num] = useState<string>("01");
	const [selectedAddr1, setSelectedAddr1] = useState<string>("서울특별시");
	...
	useEffect(() => {
		const region = async () => {
			...
			setSelectedAddr2("전체");
			onGetRegionNumber(selectedAddr1Num);
		};
		region();
	}, [selectedAddr1Num]);
